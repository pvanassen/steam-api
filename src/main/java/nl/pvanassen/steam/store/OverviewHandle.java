...
public class OverviewHandle extends DefaultHandle {
	...
	static {
		...
		try {
			...
			LoggerFactory.getLogger(MarketHistory.class).error(
	...
	public OverviewHandle(GenericHandle<OverviewItem> genericHandle,
			ObjectMapper om) {
	...
	public void handle(InputStream stream) throws IOException {
		...
		try {
			...
			NodeList nodeSet = (NodeList) ROW_XPATH.evaluate(fragment,
			...
			for (int i = 0; i < nodeSet.getLength(); i++) {
				...
				int appId = Integer.valueOf(href.substring(startAppId + 1,
						startName));
				logger.info("Found: " + urlName + ", appid: " + appId);
				Node priceSpan = (Node) PRICE_XPATH.evaluate(node,
						XPathConstants.NODE);
				int currentPrice = Integer.valueOf(priceSpan.getTextContent()
						.trim().substring(1).replace("€", "").replace(",", "")
						.replace("--", "00"));
				Node offersSpan = (Node) OFFERINGS_XPATH.evaluate(node,
						XPathConstants.NODE);
				int currentOffers = Integer.valueOf(offersSpan.getTextContent()
						.replace(",", ""));

				Node nameSpan = (Node) NAME_XPATH.evaluate(node,
						XPathConstants.NODE);
				String name = nameSpan.getTextContent();
				Node gameNameSpan = (Node) GAMENAME_XPATH.evaluate(node,
						XPathConstants.NODE);
				String gameName = gameNameSpan.getTextContent();
				genericHandle.handle(new OverviewItem(appId, name, urlName,
						currentOffers, currentPrice, gameName, steamId));
			}
