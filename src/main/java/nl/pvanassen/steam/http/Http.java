...
public class Http {
	...
	private final Map<HttpUriRequest, Long> connectionsToWatch = new HashMap<>();
	...
	private Http(String cookies, String username) {
		globalConfig = RequestConfig.custom()
				.setCookieSpec(CookieSpecs.BEST_MATCH).setSocketTimeout(10000)
				.build();
		context = HttpClientContext.create();
		this.username = username;
		cookieStore = new BasicCookieStore();
		context.setCookieStore(cookieStore);
		// Tokenize the cookie string and load all cookies into the store
		if (!"".equals(cookies)) {
			for (String cookie : cookies.split("; ")) {
				int split = cookie.indexOf('=');
				String parts[] = new String[] { cookie.substring(0, split),
						cookie.substring(split + 1) };
	...
	public void get(String url, Handle handle) throws IOException {
		...
		addHeaders(httpGet, "http://steamcommunity.com/id/" + username
				+ "/inventory/");
		handleRequest(httpGet, handle);
	}
	 ...
	 */
	public void post(String url, Map<String, String> params, Handle handle,
			String referer) throws IOException {
		HttpPost httpPost = new HttpPost(url);
		addHeaders(httpPost, referer);
		httpPost.addHeader("X-Prototype-Version", "1.7");
		...
		httpPost.setEntity(new StringEntity(sb.toString(), ContentType.create(
				"application/x-www-form-urlencoded", "UTF-8")));
		handleRequest(httpPost, handle);
	}
	
	private void handleRequest(HttpUriRequest request, Handle handle) throws IOException {
		CloseableHttpClient httpclient = getClient();
		CloseableHttpResponse response = null;
		try {
			connectionsToWatch.put(request, System.currentTimeMillis()
					+ TIMEOUT);
			response = httpclient.execute(request);
			connectionsToWatch.remove(request);
	...
	private static class WatchDog implements Runnable {
		...
		private final Map<HttpUriRequest, Long> connectionsToWatch;

		WatchDog(Map<HttpUriRequest, Long> connectionsToWatch) {
		...
		public void run() {
			...
			while (true) {
				...
				for (Map.Entry<HttpUriRequest, Long> entry : connectionsToWatch
						.entrySet()) {
					logger.info("Now: " + now + ", timeout connection: "
							+ entry.getValue() + ", waiting another "
							+ (entry.getValue() - now));
